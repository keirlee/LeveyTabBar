...
@implementation UIButton (JOBadgeViewSupport)

- (void)setBadgeVal:(int)intValue
{
    JOBadgeView * badgeView = [[JOBadgeView alloc]init];
    badgeView.showBorder = NO;
    badgeView.showShadow = NO;
    badgeView.showGloss  = NO;
    badgeView.frame = CGRectMake(badgeView.bounds.origin.x + 30, badgeView.bounds.origin.y - 5, badgeView.bounds.size.width -3, badgeView.bounds.size.height -3);
    [badgeView setAutoPositioning:NO];
    if (intValue >99) {
        intValue = 99;
    }
    NSString *strVal = [NSString stringWithFormat:@"%d",intValue];
    badgeView.badgeValue = strVal;

    if (intValue > 0) {
        [self addSubview:badgeView];
    }else{
        for (UIView *view in self.subviews) {
            if ([view isKindOfClass: [JOBadgeView class]]) {
...
- (id)initWithViewControllers:(NSArray *)vcs iconArray:(NSArray *)iconArray txtArray:(NSArray *)txtArray
{
	self = [super init];
	if (self != nil)
        {
        ...
        if (IOS7_OR_LATER) {
            ...
            _tabBar = [[LeveyTabBar alloc] initWithFrame:CGRectMake(0,
                                                                    WIN_HEIGHT - kTabBarHeight,
                                                                    320.0f,
                                                                    kTabBarHeight)
                                            iconArray:iconArray txtArray:txtArray];
        }
        else
        {
            _tabBar = [[LeveyTabBar alloc] initWithFrame:CGRectMake(0, _containerView.frame.size.height - kTabBarHeight, 320.0f, kTabBarHeight) iconArray:iconArray txtArray:txtArray];
        }
...
- (void)hidesTabBar:(BOOL)yesOrNO animated:(BOOL)animated;
{
	...
	if (animated == YES)
	{
		...
		if (yesOrNO == YES)
		{
            ...
            CGRect _transitionViewFrame = _transitionView.frame;
            _transitionViewFrame.size.height += kTabBarHeight;
            _transitionView.frame = _transitionViewFrame;
		}
		else
		{
